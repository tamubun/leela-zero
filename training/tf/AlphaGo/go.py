...
class GameState(object):
	...
	def copy(self):
		...
		done = np.zeros((self.size, self.size), np.bool)
		for x in range(self.size):
			for y in range(self.size):
				g = self.group_sets[x][y]
				if len(g) == 0:
					done[x,y] = True
					other.group_sets[x][y] = set(self.group_sets[x][y])
					other.liberty_sets[x][y] = set(self.liberty_sets[x][y])
				elif not done[x,y]:
					group_sets = set(self.group_sets[x][y])
					liberty_sets = set(self.liberty_sets[x][y])
					for (px, py) in g:
						done[px,py] = True
						other.group_sets[px][py] = group_sets
						other.liberty_sets[px][py] = liberty_sets
